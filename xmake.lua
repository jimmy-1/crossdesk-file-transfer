...
target("service_manager")
    set_kind("object")
    add_deps("rd_log")
    add_files("src/agent/service_manager.cpp")
    add_includedirs("src/agent", {public = true})
    if is_os("windows") then
        add_links("advapi32")
    end

target("crossdesk_agent")
    set_kind("binary")
    add_deps("rd_log", "common", "config_center", "minirtc", 
        "path_manager", "screen_capturer", "speaker_capturer", 
        "device_controller", "service_manager", "tools")
    add_files("src/agent/crossdesk_agent.cpp")
    add_includedirs("src/agent", {public = true})
    if is_os("windows") then
